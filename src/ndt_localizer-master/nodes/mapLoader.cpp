...
MapLoader::MapLoader(ros::NodeHandle &nh){
    ...
    pc_rviz_pub_ = nh.advertise<sensor_msgs::PointCloud2>("/downmap", 10, true);
    ...
    if (filtered_msg.width != 0) {
        filtered_msg.header.frame_id = "map";
        pc_map_pub_.publish(filtered_msg);
    }
    else
    {
        std::cout << "no filter map" << std::endl;
    }
...
//    SaveMap(filtered_scan_ptr);

    scan_ptr.reset(new pcl::PointCloud<pcl::PointXYZ>());
    filtered_scan_ptr.reset(new pcl::PointCloud<pcl::PointXYZ>());
}
...
void MapLoader::SaveMap(const pcl::PointCloud<pcl::PointXYZ>::Ptr map_pc_ptr){
    pcl::io::savePCDFile("/media/cf206/512/multi_lidars_calibration-NDT_ws/transformed_map.pcd", *map_pc_ptr);
}
