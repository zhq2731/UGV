...
{
  ROS_WARN("-------------AerialMapDisplay------------");
  std::cout <<"--------------AerialMapDisplay-----------------"<<std::endl;
  center_tile_pose_.pose.orientation.w = 1;
  
  topic_property_ =
      new RosTopicProperty("Topic", "", QString::fromStdString(ros::message_traits::datatype<localization_msgs::Localization>()),
 ...
 ROS_WARN("-------------AerialMapDisplay----111--------");
  ...
  ROS_WARN("-------------AerialMapDisplay----333--------");
  QString str(config_file.c_str());
  YAML::Node config = YAML::Load(str.toStdString());  
  if (!config)
	  ROS_WARN("-------------LoadFile error----333--------");
  else	{
      ROS_WARN("-------------LoadFile success----333--------");
	 // std::cout << config["latitude"].as<double>()<<std::endl;
  	}
  sleep(5);
  //std::cout << config["latitude"].as<double>()<<std::endl;

  // init_pos.y  = config["longitude"].as<double>();

  // init_pos.z  = config["altitude"].as<double>();

  ROS_WARN("-------------AerialMapDisplay----222--------");
  */
}
...
void AerialMapDisplay::updateMapTransformType()
{
  ...
  ROS_WARN("-----------updateMapTransformType-------------------");
  std::cout <<"-----------------updateMapTransformType-----------------------"<<std::endl;
...
void AerialMapDisplay::navFixCallback(const localization_msgs::Localization::ConstPtr &msg)
{
	...
	updateCenterTile(navSatFix);
	setStatus(StatusProperty::Ok, "Message", "NavSatFix message received");
    utm_pose.x = msg->location.pose.pose.position.x;
    utm_pose.y = msg->location.pose.pose.position.y;
}
...
void AerialMapDisplay::transformTileToMapFrame()
{
  ...
  geometry_msgs::Point t_navsat_map = utm_pose;
  /*
  ...
  */
  ...
  geometry_msgs::Point t_centertile_navsat;// = { center_tile_offset_x * tile_w_h_m, center_tile_offset_y * tile_w_h_m, 0 };
	t_centertile_navsat.x =  center_tile_offset_x * tile_w_h_m;
	t_centertile_navsat.y = center_tile_offset_y * tile_w_h_m;
	t_centertile_navsat.z = 0.0;
  center_tile_pose_.header.frame_id = map_frame_;
  center_tile_pose_.header.stamp = ref_fix_->header.stamp;
  
  center_tile_pose_.pose.position.x  = t_navsat_map.x  - t_centertile_navsat.x;
  center_tile_pose_.pose.position.y  = t_navsat_map.y  - t_centertile_navsat.y;
  center_tile_pose_.pose.position.z  = t_navsat_map.z  - t_centertile_navsat.z;
