...
class OsmFileWriter {
 public:
  static std::unique_ptr<pugi::xml_document> write(const File& osmFile) {
    ...
    lanelet::osm::OsmFileWriter::writeNodes(osmNode, osmFile.nodes, true);
 ...
 private:
  static Nodes readNodes(const pugi::xml_node& osmNode) {
    ...
    for (auto node = osmNode.child(keyword::Node); node;  // NOLINT
      ...
      const auto id = node.attribute(keyword::Id).as_llong(0);
...
std::unique_ptr<pugi::xml_document>  write(const File& osmFile) { return OsmFileWriter::write(osmFile); }


osm::File parse(const std::string& filename) {
  // read xml
  pugi::xml_document doc;
  auto result = doc.load_file(filename.c_str());
  if (!result) {
    std::cout <<"load_file error "<<std::endl;
  }
