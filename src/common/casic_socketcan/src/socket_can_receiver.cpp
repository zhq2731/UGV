#include "casic_socketcan/socket_can_common.hpp"
#include "casic_socketcan/socket_can_receiver.hpp"
...
namespace casic
{
  namespace socketcan
  {
        ...
        const std::string &str)
    {
      ...
      while (getline(input, fstr, ','))
      {
        ...
        if (std::sscanf(fstr.c_str(), "%x:%x", &filter.can_id, &filter.can_mask) == 2)
        {
          ...
          {
            filter.can_id |= CAN_EFF_FLAG; // CAN_EFF_FLAG  表示为扩展帧
          }
    ...
    {
      // std::cout << "receive11111========================= " << std::endl;
      // std::cout  <<"m_enable_fd" << m_enable_fd << std::endl;
      if (m_enable_fd == true)
      {
