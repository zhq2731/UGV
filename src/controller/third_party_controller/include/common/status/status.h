...
#ifndef COMMON_STATUS_STATUS_H_
#define COMMON_STATUS_STATUS_H_
...
public:
   ...
   */
  Status(msgs::ErrorCode code, const std::string &msg) : code_(code), msg_(msg) {}

  /**
   * @brief Create a status with the specified error code and empty msg
   * @param code the error code.
   */
  explicit Status(msgs::ErrorCode code) : code_(code), msg_("") {}
   ...
   *          false otherwise
   */
  bool ok() const { return code_ == msgs::OK; }

  /**
   * @brief get the error code
   * @returns the error code
   */
  msgs::ErrorCode code() const { return code_; }
   ...
   */
  void Save(msgs::StatusPb *status_pb) {
    if (!status_pb) {
      return;
    }
