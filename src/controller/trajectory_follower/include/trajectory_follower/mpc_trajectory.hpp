...
public:
  ...
  std::vector<geometry_msgs::Point> toPoints() const
  {
  ...
  std::vector<autoware_msgs::TrajectoryPoint> toTrajectoryPoints() const
  {
    std::vector<autoware_msgs::TrajectoryPoint> points;
    for (size_t i = 0; i < x.size(); ++i) {
      autoware_msgs::TrajectoryPoint point;
      point.pose.position.x = x.at(i);
      point.pose.position.y = y.at(i);
      point.pose.position.z = z.at(i);
      point.pose.orientation = createQuaternionFromYaw(yaw.at(i));
