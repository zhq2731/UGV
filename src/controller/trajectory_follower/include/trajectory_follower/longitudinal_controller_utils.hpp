...
using autoware_msgs::TrajectoryArrayPoint;
using autoware_msgs::TrajectoryPoint;
  ...
  const T & points, const Pose & pose, const double max_dist, const double max_yaw)
{
  ...
  const size_t seg_idx =
    findFirstNearestSegmentIndexWithSoftConstraints(points, pose, max_dist, max_yaw);

  const double len_to_interpolated =
    calcLongitudinalOffsetToSegment(points, seg_idx, pose.position);
