...
struct MPCData
{
  int nearest_idx;//最近点在参考线的索引
  double nearest_time;//插值得到的最近点时间戳
  geometry_msgs::Pose nearest_pose;//插值得到的最近点
  geometry_msgs::Pose current_pose;
  double steer;//当前车辆的前轮转角
  double predicted_steer;
  double lateral_err;//横向偏移误差
  double yaw_err;    //航向误差
};
/**
...
private:
 ...
 // rclcpp::Clock::SharedPtr m_clock = std::make_shared<rclcpp::Clock>(RCL_ROS_TIME);
  ...
  std::vector<autoware_msgs::AckermannLateralCommand> m_ctrl_cmd_vec;
   ...
   */
  void resetPrevResult(const autoware_msgs::SteeringReport & current_steer);
  /**
  ...
  trajectory_follower::MPCTrajectory applyVelocityDynamicsFilter(
    ...
    const geometry_msgs::Pose & current_pose, const double v0) const;
  /**
