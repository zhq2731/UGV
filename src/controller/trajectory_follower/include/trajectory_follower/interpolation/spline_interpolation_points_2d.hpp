...
#include "trajectory_follower/interpolation/spline_interpolation.hpp"
...
public:
  ...
  {
    std::vector<geometry_msgs::Point> points_inner;
    for (const auto & p : points) {
      points_inner.push_back(getPoint(p));
    }
  ...
  geometry_msgs::Point getSplineInterpolatedPoint(const size_t idx, const double s) const;
  double getSplineInterpolatedYaw(const size_t idx, const double s) const;

  double getAccumulatedLength(const size_t idx) const;

private:
  void calcSplineCoefficientsInner(const std::vector<geometry_msgs::Point> & points);
