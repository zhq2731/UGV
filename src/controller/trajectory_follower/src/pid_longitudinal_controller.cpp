...
: node_{&node}
{
  ...
  m_delay_compensation_time = node_->declare_parameter("delay_compensation_time").get<double>();  // [s]

  // parameters to enable functions
  m_enable_smooth_stop = node_->declare_parameter("enable_smooth_stop").get<bool>();
  m_enable_overshoot_emergency = node_->declare_parameter("enable_overshoot_emergency").get<bool>();
  m_enable_large_tracking_error_emergency =
    node_->declare_parameter("enable_large_tracking_error_emergency").get<bool>();
  m_enable_slope_compensation = node_->declare_parameter("enable_slope_compensation").get<bool>();
  m_enable_keep_stopped_until_steer_convergence =
    node_->declare_parameter("enable_keep_stopped_until_steer_convergence").get<bool>();

  // parameters for state transition
  {
    auto & p = m_state_transition_params;
    // drive
    p.drive_state_stop_dist = node_->declare_parameter("drive_state_stop_dist").get<double>();  // [m]
    p.drive_state_offset_stop_dist =
      node_->declare_parameter("drive_state_offset_stop_dist").get<double>();  // [m]
    // stopping
    p.stopping_state_stop_dist =
      node_->declare_parameter("stopping_state_stop_dist").get<double>();  // [m]
    p.stopped_state_entry_duration_time =
      node_->declare_parameter("stopped_state_entry_duration_time").get<double>();  // [s]
    // stop
    p.stopped_state_entry_vel =
      node_->declare_parameter("stopped_state_entry_vel").get<double>();  // [m/s]
    p.stopped_state_entry_acc =
      node_->declare_parameter("stopped_state_entry_acc").get<double>();  // [m/s²]

    // emergency
    p.emergency_state_overshoot_stop_dist =
      node_->declare_parameter("emergency_state_overshoot_stop_dist").get<double>();  // [m]
    p.emergency_state_traj_trans_dev =
      node_->declare_parameter("emergency_state_traj_trans_dev").get<double>();  // [m]
    p.emergency_state_traj_rot_dev =
      node_->declare_parameter("emergency_state_traj_rot_dev").get<double>();  // [m]
  }

  // parameters for drive state
  {
    // initialize PID gain
    const double kp{node_->declare_parameter("kp").get<double>()};
    const double ki{node_->declare_parameter("ki").get<double>()};
    const double kd{node_->declare_parameter("kd").get<double>()};
    m_pid_vel.setGains(kp, ki, kd);

    // initialize PID limits
    const double max_pid{node_->declare_parameter("max_out").get<double>()};     // [m/s^2]
    const double min_pid{node_->declare_parameter("min_out").get<double>()};     // [m/s^2]
    const double max_p{node_->declare_parameter("max_p_effort").get<double>()};  // [m/s^2]
    const double min_p{node_->declare_parameter("min_p_effort").get<double>()};  // [m/s^2]
    const double max_i{node_->declare_parameter("max_i_effort").get<double>()};  // [m/s^2]
    const double min_i{node_->declare_parameter("min_i_effort").get<double>()};  // [m/s^2]
    const double max_d{node_->declare_parameter("max_d_effort").get<double>()};  // [m/s^2]
    const double min_d{node_->declare_parameter("min_d_effort").get<double>()};  // [m/s^2]
    m_pid_vel.setLimits(max_pid, min_pid, max_p, min_p, max_i, min_i, max_d, min_d);

    // set lowpass filter for vel error and pitch
    const double lpf_vel_error_gain{node_->declare_parameter("lpf_vel_error_gain").get<double>()};
    m_lpf_vel_error =
      std::make_shared<trajectory_follower::LowpassFilter1d>(0.0, lpf_vel_error_gain);

    m_current_vel_threshold_pid_integrate =
      node_->declare_parameter("current_vel_threshold_pid_integration").get<double>();  // [m/s]

    m_enable_brake_keeping_before_stop =
      node_->declare_parameter("enable_brake_keeping_before_stop").get<bool>();         // [-]
    m_brake_keeping_acc = node_->declare_parameter("brake_keeping_acc").get<double>();  // [m/s^2]
  }

  // parameters for smooth stop state
  {
    const double max_strong_acc{
      node_->declare_parameter("smooth_stop_max_strong_acc").get<double>()};  // [m/s^2]
    const double min_strong_acc{
      node_->declare_parameter("smooth_stop_min_strong_acc").get<double>()};                // [m/s^2]
    const double weak_acc{node_->declare_parameter("smooth_stop_weak_acc").get<double>()};  // [m/s^2]
    const double weak_stop_acc{
      node_->declare_parameter("smooth_stop_weak_stop_acc").get<double>()};  // [m/s^2]
    const double strong_stop_acc{
      node_->declare_parameter("smooth_stop_strong_stop_acc").get<double>()};  // [m/s^2]

    const double max_fast_vel{
      node_->declare_parameter("smooth_stop_max_fast_vel").get<double>()};  // [m/s]
    const double min_running_vel{
      node_->declare_parameter("smooth_stop_min_running_vel").get<double>()};  // [m/s]
    const double min_running_acc{
      node_->declare_parameter("smooth_stop_min_running_acc").get<double>()};  // [m/s^2]
    const double weak_stop_time{
      node_->declare_parameter("smooth_stop_weak_stop_time").get<double>()};  // [s]

    const double weak_stop_dist{
      node_->declare_parameter("smooth_stop_weak_stop_dist").get<double>()};  // [m]
    const double strong_stop_dist{
      node_->declare_parameter("smooth_stop_strong_stop_dist").get<double>()};  // [m]
  ...
  // parameters for stop state
  {
    ...
    p.vel = node_->declare_parameter("stopped_vel").get<double>();    // [m/s]
    p.acc = node_->declare_parameter("stopped_acc").get<double>();    // [m/s^2]
    p.jerk = node_->declare_parameter("stopped_jerk").get<double>();  // [m/s^3]
  }

  // parameters for emergency state
  {
    auto & p = m_emergency_state_params;
    p.vel = node_->declare_parameter("emergency_vel").get<double>();    // [m/s]
    p.acc = node_->declare_parameter("emergency_acc").get<double>();    // [m/s^2]
    p.jerk = node_->declare_parameter("emergency_jerk").get<double>();  // [m/s^3]
  }

  // parameters for acceleration limit
  m_max_acc = node_->declare_parameter("max_acc").get<double>();  // [m/s^2]
  m_min_acc = node_->declare_parameter("min_acc").get<double>();  // [m/s^2]

  // parameters for jerk limit
  m_max_jerk = node_->declare_parameter("max_jerk").get<double>();  // [m/s^3]
  m_min_jerk = node_->declare_parameter("min_jerk").get<double>();  // [m/s^3]

  // parameters for slope compensation
  m_use_traj_for_pitch = node_->declare_parameter("use_trajectory_for_pitch_calculation").get<bool>();
  const double lpf_pitch_gain{node_->declare_parameter("lpf_pitch_gain").get<double>()};
  m_lpf_pitch = std::make_shared<trajectory_follower::LowpassFilter1d>(0.0, lpf_pitch_gain);
  m_max_pitch_rad = node_->declare_parameter("max_pitch_rad").get<double>();  // [rad]
  m_min_pitch_rad = node_->declare_parameter("min_pitch_rad").get<double>();  // [rad]
  ...
  m_ego_nearest_dist_threshold =
    node_->has_parameter("ego_nearest_dist_threshold")
      ...
      : node_->declare_parameter("ego_nearest_dist_threshold").get<double>();  // [m]
  m_ego_nearest_yaw_threshold =
    node_->has_parameter("ego_nearest_yaw_threshold")
      ? node_->get_parameter("ego_nearest_yaw_threshold").as_double()
      : node_->declare_parameter("ego_nearest_yaw_threshold").get<double>();  // [rad]
