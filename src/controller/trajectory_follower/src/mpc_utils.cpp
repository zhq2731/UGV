...
bool convertToMPCTrajectory(
  const autoware_msgs::TrajectoryPointArray  & input, MPCTrajectory & output)
{
  output.clear();
  for (const autoware_msgs::TrajectoryPoint & p : input.points) {
...
bool convertToAutowareTrajectory(
  const MPCTrajectory & input, autoware_msgs::TrajectoryPointArray & output)
{
  output.points.clear();
  autoware_msgs::TrajectoryPoint p;
  ...
  for (size_t i = 0; i < input.size(); ++i) {
    ...
    p.pose.orientation = createQuaternionFromYaw(input.yaw.at(i));
...
//计算每一个参考线上每一个点，相对于第一个点，到达时候的相对时间
...
bool calcNearestPoseInterp(
  ...
  geometry_msgs::Pose * nearest_pose, size_t * nearest_index, double * nearest_time,
  const double max_dist, const double max_yaw)
{
  ...
  *nearest_index = findFirstNearestIndexWithSoftConstraints(
  ...
  auto calcSquaredDist =
    [](const geometry_msgs::Pose & p, const MPCTrajectory & t, const size_t idx) {
  ...
  const double tmp_yaw_err = wrap_angle(
    traj.yaw[*nearest_index] - traj.yaw[second_nearest_index]);
  const double nearest_yaw = wrap_angle(
...
double calcStopDistance(
  const autoware_msgs::TrajectoryPointArray & current_trajectory, const int origin)
{
