...
bool ReferenceLineInfo::Init(const std::vector<const Obstacle*>& obstacles) {
  const auto param   = vehicle_info_util::VehicleInfoUtil::get_instance();
  // stitching point
  const auto& path_point = adc_planning_point_.path_point();
  //规划的起点
  Vec2d position(path_point.x(), path_point.y());

  //从后轴中心到车辆几何中心的向量(以车体的前左上为车体坐标系)
  Vec2d vec_to_center(
      (param->front_overhang_m + param->wheel_base_m - param->rear_overhang_m) / 2.0, 0.0);

  //车辆在规划起点的时候，车辆的几何中心的全局坐标
  Vec2d center(position + vec_to_center.rotate(path_point.theta()));

  //车辆在规划起点的时候，车辆box
  Box2d box(center, path_point.theta(), param->vehicle_length_m, param->vehicle_width_m);
  ...
  //车辆当前位置的box
  Box2d vehicle_box(vehicle_center, vehicle_state_.heading, param->vehicle_length_m,
                    param->vehicle_width_m);
  
  if (!reference_line_.GetSLBoundary(box, &adc_sl_boundary_)) {
    std::cout  << "Failed to get ADC boundary from box: " ;
    return false;
  }

  //std::cout <<"start_s = ---"<<adc_sl_boundary_.start_s()<<"  end_s = "<<adc_sl_boundary_.end_s()<<std::endl;
  //std::cout <<"start_l = ---"<<adc_sl_boundary_.start_l()<<"  end_l = "<<adc_sl_boundary_.end_l()<<std::endl;
  //std::cout <<"ref len  = ---"<<reference_line_.Length()<<std::endl;
  //InitFirstOverlaps();

  if (adc_sl_boundary_.end_s() < 0 ||
      adc_sl_boundary_.start_s() > reference_line_.Length()) {
      std::cout << "Vehicle SL " << " is not on reference line:[0, " << std::endl;
  }
  ...
  if (adc_sl_boundary_.start_l() > kOutOfReferenceLineL ||
      ...
      std::cout << "Ego vehicle is too far away from reference line.";
      return false;
  }
  ...
  if (!AddObstacles(obstacles)) {
    std::cout  << "Failed to add obstacles to reference line";
    return false;
  }
...
double ReferenceLineInfo::GetCruiseSpeed() const {
 ...
 // PlanningConfig *planning_config = PlanningConfig::get_instance();
  //return cruise_speed_ > 0.0 ? cruise_speed_ : planning_config->default_cruise_speed;
}
...
Obstacle* ReferenceLineInfo::AddObstacle(const Obstacle* obstacle) {
  ...
  */
  ...
  if (mutable_obstacle->IsLaneBlocking()) {
      std::cout << "obstacle [" << obstacle->Id() << "] is lane blocking."<<std::endl;
  } else {
      //std::cout << "obstacle [" << obstacle->Id() << "] is NOT lane blocking.";
  }
  ...
  if (IsIrrelevantObstacle(*mutable_obstacle)) {
   ...
   // ignore.mutable_ignore();
    path_decision_.AddIgnoreDecision( obstacle->Id());
    //path_decision_.AddLongitudinalDecision("reference_line_filter",
                                          // obstacle->Id(), ignore);
    //ADEBUG << "NO build reference line st boundary. id:" << obstacle->Id();
  } else {
     dynamic_obs_num_ += (!mutable_obstacle->IsStatic());
	 total_obs_num_++;
	 if (5 == obstacle->Perception().type )
	 	have_nagative_obs = true;

	 if (1 == obstacle->Perception().type )
	 	have_dynamic_car = true;
...
bool ReferenceLineInfo::IsIrrelevantObstacle(const Obstacle& obstacle) {
  ...
  if (!reference_line_.IsOnLane(obstacle_boundary))
      return true;

  
  //障碍物在车后面
  //if (is_on_reference_line_ /*&& !IsChangeLanePath()*/ &&
  //    obstacle_boundary.end_s() < adc_sl_boundary_.end_s() && //lianbin,感觉这里是错的，adc_sl_boundary_应该取的是start.s()?
      //lianbin ,源码应该是错误的，这里应该取反
...
int ReferenceLineInfo::MakeMainStopDecision(
  ...
  for (const auto* obstacle : path_decision_.obstacles().Items()) {
    ...
    ugv::common::PointENU stop_point = object_decision.stop().stop_point();
