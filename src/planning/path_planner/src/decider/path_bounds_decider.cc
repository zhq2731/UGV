...
bool PathBoundsDecider::Process(
  ...
  bool ret_display = GetLaneBoundryForDisPlay(*reference_line_info,&lane_bound);
  
  std::vector<std::pair<double, double>> display_path_bound_pair;
  for (size_t i = 0; i < lane_bound.size(); ++i) {
	display_path_bound_pair.emplace_back(std::get<1>(lane_bound[i]), std::get<2>(lane_bound[i]));
  }
  if (ret_display){
      PathBoundary lane_boundry(std::get<0>(lane_bound[0]),
                                           planning_config->kPathBoundsDeciderResolution,display_path_bound_pair); 			   
      reference_line_info->addDiplayBoundry(lane_boundry) ;
  }
  ...
  for (const auto& lane_borrow_info : lane_borrow_info_list) {
    ...
    bool ret = GenerateRegularPathBound(
    ...
    candidate_path_boundaries.emplace_back(std::get<0>(regular_path_bound[0]),
                                           planning_config->kPathBoundsDeciderResolution,
    ...
    candidate_path_boundaries.back().set_label(std::string("regular/self"));
  ...
  if (candidate_path_boundaries.empty()){
  	  std::cout <<"error: candidate_path_boundaries is empty"<<std::endl;
	  return false;
  }
...
void PathBoundsDecider::InitPathBoundsDecider(
  ...
  TrajectoryPoint planning_start_point = frame.PlanningStartPoint();
  ...
  //std::cout <<" adc_frenet_s_ ---"<<adc_frenet_s_<<std::endl;
  //std::cout <<" adc_frenet_l_ ---"<<adc_frenet_l_<<std::endl;
 ...
 // reference_line.GetOffsetToMap(adc_frenet_s_, &offset_to_map);
  ...
  if (!reference_line.GetLaneWidth(adc_frenet_s_, &lane_left_width,
                                   &lane_right_width)) {
      adc_lane_width_ = planning_config->kDefaultRoadWidth;
  } else {
...
double PathBoundsDecider::GetBufferBetweenADCCenterAndEdge() {
  ...
  double adc_half_width =
      vehicle_param->vehicle_width_m / 2.0;
...
bool PathBoundsDecider::UpdatePathBoundaryWithBuffer(
  ...
  double left_adc_buffer_coeff  = planning_config->boundryConfig.adc_buffer_coeff;

  double right_adc_buffer_coeff = planning_config->boundryConfig.adc_buffer_coeff;
  ...
  if (new_l_min > new_l_max) {
    std::cout  << "11 Path is blocked at idx = " << idx <<std::endl;
    return false;
  }
...
bool PathBoundsDecider::UpdatePathBoundaryAndCenterLineWithBuffer(
  ...
  if(!UpdatePathBoundaryWithBuffer(idx, left_bound, right_bound, path_boundaries))
      return false;
...
bool PathBoundsDecider::UpdatePathBoundary(size_t idx, double left_bound,
  ...
  // If blocked, don't update anything, return false.
  if (new_l_min > new_l_max) {
    std::cout << "Path is blocked at idx = " << idx<<std::endl;
    return false;
  }
  // Otherwise, update path_boundaries and center_line; then return true.
  std::get<1>((*path_boundaries)[idx]) = new_l_min;
...
void PathBoundsDecider::TrimPathBounds(const int path_blocked_idx,
  ...
  if (path_blocked_idx != -1) {
    if (path_blocked_idx == 0) {
      std::cout  << "Completely blocked. Cannot move at all."<<std::endl;;
    }
...
bool PathBoundsDecider::checkBlock(int idx,PathBound* const path_boundaries,double left_bound,double right_bound )
{
	double new_l_min =
	  std::fmax(std::get<1>((*path_boundaries)[idx]),
...
bool PathBoundsDecider::GetBoundaryFromStaticObstacles(
  ...
  //(is_start_s, s, l_min, l_max, obstacle_id)
  auto sorted_obstacles = SortObstaclesForSweepLine(indexed_obstacles);
 // ADEBUG << "There are " << sorted_obstacles.size() << " obstacles.";
  double center_line = adc_frenet_l_; //规划起点的l
  
  size_t obs_idx = 0;
  int path_blocked_idx = -1;
  std::multiset<double, std::greater<double>> right_bounds; //从大到小排列
  right_bounds.insert(std::numeric_limits<double>::lowest());
  std::multiset<double> left_bounds; //从小到大排列
    ...
    if (obs_idx < sorted_obstacles.size() &&
	  ...
	  while (obs_idx < sorted_obstacles.size() &&
        ...
        if (std::get<0>(curr_obstacle) == 1) {
          ...
          //这里感觉有问题：假设两个车道，车在左边行驶，左边车道前方有个障碍物
          //障碍物的l中心小于center_line，则判断为应该从左边行驶，而实际上可能左边
          //并不能行驶，这里应该是检查是否可通行，如果不行，应该再判断右边

		  bool pass_right = (0.5*(curr_obstacle_l_min + curr_obstacle_l_max) >= center_line)?true:false;;
		  bool pass_left  = (0.5*(curr_obstacle_l_min + curr_obstacle_l_max) < center_line)?true:false;
		  bool pass_right_unblock = false;
		  bool pass_left_unblock = false;
		  if (pass_left && (!checkBlock(i,path_boundaries,*left_bounds.begin(), curr_obstacle_l_max))) 
		      pass_left_unblock = true;

		  if (pass_right && (!checkBlock(i,path_boundaries,curr_obstacle_l_min, *right_bounds.begin()))) 
	          pass_right_unblock = true;
		  
		  if (pass_left_unblock) {
        ...
        if (std::get<1>((*path_boundaries)[i]) >
            std::get<2>((*path_boundaries)[i])) {
          std::cout << "1 Path is blocked at s = " << curr_s<<std::endl;
          path_blocked_idx = static_cast<int>(i);
      ...
      if (std::get<1>((*path_boundaries)[i]) >
          std::get<2>((*path_boundaries)[i])) {
        std::cout  << "2 Path is blocked at s = " << curr_s <<std::endl;
        path_blocked_idx = static_cast<int>(i);
        if (!obs_id_to_direction.empty()) {
          *blocking_obstacle_id = obs_id_to_direction.begin()->first;
        }
...
bool PathBoundsDecider::InitPathBoundary(
  ...
  for (double curr_s = adc_frenet_s_;
       curr_s < reference_line.Length();
       curr_s += planning_config->kPathBoundsDeciderResolution) {
  ...
  // Return.
  if (path_bound->empty()) {
    std::cout << "Empty path boundary in InitPathBoundary"<<std::endl;
    return false;
  }
...
bool PathBoundsDecider::GenerateRegularPathBound(
  ...
  while (!blocking_obstacle_id->empty() &&
         ...
         counter < planning_config->kNumExtraTailBoundPoint) {
...
std::vector<ObstacleEdge> PathBoundsDecider::SortObstaclesForSweepLine(
  ...
  for (const Obstacle* obstacle : indexed_obstacles.Items()) {
    ...
    double lon_start_buffer = planning_config->obstacle_lon_start_buffer;
    double lon_end_buffer = planning_config->obstacle_lon_end_buffer;
    double lat_buffer = planning_config->obstacle_lat_buffer;
	sorted_obstacles.emplace_back(
        1, obstacle_sl.start_s() - lon_start_buffer,
        obstacle_sl.start_l() - lat_buffer,
        obstacle_sl.end_l() + lat_buffer, obstacle->Id());

	sorted_obstacles.emplace_back(
        0, obstacle_sl.end_s() + lon_end_buffer,
        obstacle_sl.start_l() - lat_buffer,
        obstacle_sl.end_l() + lat_buffer, obstacle->Id());
  ...
  // Sort.优先按s排序，在s相等的情况下，is_start_s 为1的排在前面
...
bool PathBoundsDecider::GenerateFallbackPathBound(
    const ReferenceLineInfo& reference_line_info, PathBound* const path_bound) {
  // 1. Initialize the path boundaries to be an indefinitely large area.
  if (!InitPathBoundary(reference_line_info, path_bound)) {
...
bool PathBoundsDecider::GetLaneBoundryForDisPlay(
    const ReferenceLineInfo& reference_line_info,PathBound* const lane_bound)
{
	PlanningConfig *planning_config = PlanningConfig::get_instance();
	// 1. Initialize the path boundaries to be an indefinitely large area.
	if (!InitPathBoundary(reference_line_info, lane_bound)) {
	    const std::string msg = "Failed to initialize path boundaries.";
	    // AERROR << msg;
	    //return Status(ErrorCode::PLANNING_ERROR, msg);
	    return false;
	...
	for (size_t i = 0; i < lane_bound->size(); ++i) {
		...
		if (!reference_line.GetLaneWidth(curr_s, &leftBound,
									   &rightBound)) {
		    leftBound = planning_config->kDefaultLaneWidth/2.0;
		    rightBound = planning_config->kDefaultLaneWidth/2.0;
		}
        std::get<1>((*lane_bound)[i]) = -rightBound;
        std::get<2>((*lane_bound)[i]) = leftBound;			   
	}
	return true;
...
bool PathBoundsDecider::GetBoundaryFromLanesAndADC(
  ...
  for (size_t i = 0; i < path_bound->size(); ++i) {
    ...
    if (!UpdatePathBoundaryWithBuffer(i, curr_lane_left_width, -curr_lane_right_width,
...
TrajectoryPoint PathBoundsDecider::InferFrontAxeCenterFromRearAxeCenter(
