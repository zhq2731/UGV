...
#include "optimizer/piecewise_jerk_problem.h"
...
void PiecewiseJerkProblem::CalculateAffineConstraint(
  ...
  assert(constraint_index== num_of_variables);
  ...
  assert(constraint_index == num_of_constraints);
...
OSQPSettings* PiecewiseJerkProblem::SolverDefaultSettings() {
  ...
  settings->verbose = planning_config->enable_osqp_debug;
...
void PiecewiseJerkProblem::set_x_ref(const double weight_x_ref,
  ...
  assert(x_ref.size() == num_of_knots_);
...
void PiecewiseJerkProblem::set_x_ref(std::vector<double> weight_x_ref_vec,
                                     std::vector<double> x_ref) {
  assert(x_ref.size()== num_of_knots_);
  assert(weight_x_ref_vec.size()==num_of_knots_);
