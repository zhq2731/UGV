...
class PiecewiseJerkPathOptimizer:public PathOptimizer   {
 ...
 bool Process( ReferenceLineInfo* reference_line_info_,
   const TrajectoryPoint& init_point, const bool path_reusable) override;

 private:


/*
  TrajectoryPoint InferFrontAxeCenterFromRearAxeCenter(
      const TrajectoryPoint& traj_point);

  std::vector<PathPoint> ConvertPathPointRefFromFrontAxeToRearAxe(
